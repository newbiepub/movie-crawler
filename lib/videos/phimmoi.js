...
class Phimmoi {
    ...
    async crawlMediaSource(url) {
        try {
            ...
            console.log("GET MEDIA URL - ", getMediaUrl);
    ...
    async crawlMediaUrl(url) {
        try {
            let getMediaUrl = await this.findMediaServer(url);
            let movie = await this.getMediaUrls(getMediaUrl, {});
        ...
        return new Promise((resolve, reject) => {
            request(media, this.requestOption, (err, res, jsonString) => {
                if (!err && res.statusCode === 200) {
                    ...
                    console.log("DECODED LINK: ", this.decodeAES(body.url, password));
                    try {
                        // Check medias empty
                        if ((body.medias || []).length) {
                            ...
                            // response has url property - push url into mediaItems
                        } else if (body.url) {
                            mediaItems.push({
                                type: "video",
                                width: 1280,
                                height: 720,
                                resolution: 720,
                                url: this.decodeAES(body.url, password)
                            })
                        }
                    } catch (e) {
                        reject(e);
    ...
    findMedia(url) {
        return new Promise((resolve, reject) => {
            request(`${this.baseUrl}${url}xem-phim.html`, this.requestOption, async (err, res, html) => {
                if (!err && res.statusCode === 200) {
                    try {
                        let $ = cheerio.load(html);
                        let media = await this.unpackingMovie(html);
                        if (media != undefined) {
    ...
    findMediaServer(url) {
        return new Promise((resolve, reject) => {
            request(`${this.baseUrl}${url}`, this.requestOption, async (err, res, html) => {
                if (!err && res.statusCode === 200) {
                    try {
                        let $ = cheerio.load(html);
                        let media = await this.unpackingMovie(html);
